...
VALUES('Programação', 'programacao', 'Programe nas principais linguagens e plataformas. Iniciantes são bem vindos nos cursos de lógica e JavaScript.', '', true, 1, 'https://www.alura.com.br/assets/api/formacoes/categorias/512/programacao-transparent.png', '#00c86f');

INSERT INTO
`Category`(`name`, `code`, `description`, study_guide, `active`, `sequence`, url_image, hex_code)
VALUES('DevOps', 'devops', 'Aprenda Git. Entenda a entrega contínua. Estude Linux. Gerencie servidores na nuvem. Explore o mundo de Internet das coisas e da robótica.', '', true, 2, 'https://www.alura.com.br/assets/api/formacoes/categorias/512/devops-transparent.png', '#f16165');

INSERT INTO
`Category`(`name`, `code`, `description`, study_guide, `active`, `sequence`, url_image, hex_code)
VALUES('Business', 'business', 'Agilidade. Práticas de gestão. Vendas. Liderança.', '', false, 0, 'https://www.alura.com.br/assets/api/formacoes/categorias/512/inovacao-gestao-transparent.png', '#ff8c2a');

INSERT INTO
`Subcategory`(`name`,`code`,`description`,study_guide, `active`, sequence, category_id)
VALUES('Java', 'java', 'Java é uma grande plataforma presente em todo lugar: de corporações à bancos e governo. Desenvolva aplicações robustas com um back-end e construa APIs.', '', true, 1, (SELECT `id` FROM Category WHERE `code` = 'programacao'));

INSERT INTO
`Subcategory`(`name`,`code`,`description`,study_guide, `active`, sequence, category_id)
VALUES('Java e Persistência', 'java-e-persistencia', '', '', true, 2, (SELECT `id` FROM Category WHERE `code` = 'programacao'));

INSERT INTO
`Subcategory`(`name`,`code`,`description`,study_guide, `active`, sequence, category_id)
VALUES('PHP', 'php', 'PHP é uma das linguagens mais utilizadas.', '', true, 3, (SELECT `id` FROM Category WHERE `code` = 'programacao'));

INSERT INTO
`Subcategory`(`name`,`code`,`description`,study_guide, `active`, sequence, category_id)
VALUES('COBOL', 'cobol', '', '', false, 0, (SELECT `id` FROM Category WHERE `code` = 'programacao'));

INSERT INTO
`Subcategory`(`name`,`code`,`description`,study_guide, `active`, sequence, category_id)
VALUES('Builds e Controle de versão', 'builds-e-controle-de-versao', 'As ferramentas mais utilizadas para desenvolvimento: controle de versão com Git e Github além de build da aplicação através de Maven.', '', true, 1, (SELECT `id` FROM Category WHERE `code` = 'devops'));
